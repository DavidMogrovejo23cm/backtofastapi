...
export class UserService {
  ...
  async create(createUserDto: CreateUserDto) {
    try {
      ...
      // Crear usuario
      const userData = {
        name: createUserDto.name || createUserDto.email, // Usar name o email como fallback
        email: createUserDto.email.trim(),
        password: hashedPassword,
        role: createUserDto.role || 'EMPLEADO'
      };
  ...
  async update(id: number, updateUserDto: UpdateUserDto) {
    try {
      ...
      // Si se está actualizando email, verificar que no exista
